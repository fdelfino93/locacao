...
export const ModernLocatarioFormV3: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        setMessage({ type: 'success', text: response.message || 'Locatário cadastrado com sucesso!' });
        // Reset form após sucesso...
      } else {
        setMessage({ type: 'error', text: 'Erro ao cadastrar locatário' });
      }
    } catch (error: any) {
      const errorMessage = error.response?.data?.detail || 'Erro ao cadastrar locatário';
        ...
        >
          ...
          >
            ...
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
              Preencha os dados do locatário para cadastro completo no sistema
            </p>
          </motion.div>
