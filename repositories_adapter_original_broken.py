...
def buscar_faturas(filtros=None, page=1, limit=20, order_by='data_vencimento', order_dir='DESC'):
    ...
    try:
        ...
        # Se houver prestações, retornar elas
        if faturas:
            print(f"📊 Retornando {len(faturas)} prestações de contas do banco")
            return {
                'success': True,
        ...
        # Se não houver prestações no banco, retornar dados de teste
        else:
            print("📊 Sem prestações no banco, retornando dados de teste")
            faturas_teste = [
        {
            'id': 1,
            'numero_fatura': 'FAT-001',
...
def buscar_locadores_contrato(contrato_id):
    ...
    try:
        ...
        # Primeiro, tenta buscar locadores específicos do contrato
        cursor.execute("""
            SELECT 
                cl.id,
                cl.contrato_id,
        ...
        # Se não encontrou locadores específicos, busca o locador do imóvel
        if not rows:
            cursor.execute("""
                SELECT 
                    NULL as id,
